...
describe('sync-prettier-config command', () => {
  ...
  it('fails when root package.json does not exist', async () => {
    await withTempDir('agent-toolkit-sync-prettier-config-missing-pkg-', async (tempDir) => {
      const result = runCli(['sync-prettier-config', '--cwd', tempDir]);
  ...
  it('fails when root package.json is invalid JSON', async () => {
    await withTempDir('agent-toolkit-sync-prettier-config-invalid-pkg-', async (tempDir) => {
  ...
  it('supports --dry-run without mutating files', async () => {
    await withTempDir('agent-toolkit-sync-prettier-config-dry-run-', async (tempDir) => {
      await writeTextFile(path.join(tempDir, 'package.json'), '{"name":"tmp"}\n');

      const result = runCli(['sync-prettier-config', '--cwd', tempDir, '--dry-run']);
      assert.equal(result.status, 0);
  ...
  it('supports --json output report file', async () => {
    await withTempDir('agent-toolkit-sync-prettier-config-json-', async (tempDir) => {
      await writeTextFile(path.join(tempDir, 'package.json'), '{"name":"tmp"}\n');
  ...
  it('supports --check and --dry-run together with check taking precedence', async () => {
    await withTempDir('agent-toolkit-sync-prettier-config-check-dry-run-', async (tempDir) => {
      await writeTextFile(path.join(tempDir, 'package.json'), '{"name":"tmp"}\n');

      const result = runCli(['sync-prettier-config', '--cwd', tempDir, '--check', '--dry-run']);
      assert.equal(result.status, 2);
