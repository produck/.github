...
export function runValidateCommitMsg(options) {
  ...
  // [PUBLISH] is generated by lerna and is always a single-line repo-wide tag.
  // It does not belong to any package scope, so section headers are not required.
  // Summary text is also not required for this special automated tag.
  const isPublishOnlyMessage = lines.length === 1 && /^\[PUBLISH\](\s+.*)?$/.test(lines[0].trim());
