...
describe('sync-lint command', () => {
  ...
  it('supports --check mode without mutating files', async () => {
    await withTempDir('agent-toolkit-sync-lint-check-', async (tempDir) => {
      ...
      assert.equal(
        fs.existsSync(path.join(tempDir, 'eslint.config.mjs')),
        false,
      );
    });
  });
  ...
  it('fails when root package.json does not exist', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-missing-pkg-',
      async (tempDir) => {
        const result = runCli(['sync-lint', '--cwd', tempDir]);
        assert.equal(result.status, 2);
  ...
  it('fails when root package.json is invalid JSON', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-invalid-pkg-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{ invalid json\n',
  ...
  it('marks unpatchable eslint config and exits non-zero in sync mode', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-unpatchable-sync-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
  ...
  it('marks unpatchable eslint config and exits non-zero in check mode', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-unpatchable-check-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
  ...
  it('marks unpatchable eslint config and exits non-zero in dry-run mode', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-unpatchable-dry-run-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
  ...
  it('uses check mode when both --check and --dry-run are provided', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-check-dry-run-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
  ...
  it('resolves @produck/eslint-rules version without consulting the npm registry', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-offline-version-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
  ...
  it('is a no-op on second run after state is synchronized', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-no-op-second-run-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
        ...
        const afterConfig = fs.readFileSync(
          path.join(tempDir, 'eslint.config.mjs'),
          'utf8',
        );
  ...
  it('marks config unpatchable when import exists but export default array is missing', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-unpatchable-import-only-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
  ...
  it('passes in --check mode when project is already synchronized', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-check-clean-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
  ...
  it('patches config without trailing newline and writes final newline', async () => {
    await withTempDir(
      'agent-toolkit-sync-lint-patch-newline-',
      async (tempDir) => {
        await writeTextFile(
          path.join(tempDir, 'package.json'),
          '{"name":"tmp"}\n',
