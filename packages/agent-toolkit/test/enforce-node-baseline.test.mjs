...
describe('enforce-node-baseline command', () => {
  ...
  it('runs preflight, sync-instructions, sync-editorconfig, sync-format, sync-lint, sync-install, sync-git, sync-coverage, and sync-publish in order', async () => {
    await withTempDir(
      'agent-toolkit-enforce-node-baseline-sync-',
      async (tempDir) => {
...
`,
        ...
        const copiedInstructionText = fs.readFileSync(
          copiedInstruction,
          'utf8',
        );
        ...
        assert.equal(
          workspacePackage.scripts['produck:coverage'],
          REQUIRED_COVERAGE_SCRIPT,
        );
      },
    );
  });
  ...
  it('supports check mode as non-mutating flow and exits non-zero on first mismatch', async () => {
    await withTempDir(
      'agent-toolkit-enforce-node-baseline-check-',
      async (tempDir) => {
        const sourceDir = path.join(tempDir, 'source');
        await writeTextFile(
  ...
  it('supports --json output for report file', async () => {
    await withTempDir(
      'agent-toolkit-enforce-node-baseline-json-',
      async (tempDir) => {
        const sourceDir = path.join(tempDir, 'source');
        await writeTextFile(
          path.join(sourceDir, '00-sample.instructions.md'),
...
`,
  ...
  it('fails when --cwd does not exist', () => {
    const missingCwd = path.resolve(
      'D:/tmp/agent-toolkit-enforce-node-baseline-missing-cwd',
    );
  ...
  it('supports --dry-run with forwarded --workspace and --force flags', async () => {
    await withTempDir(
      'agent-toolkit-enforce-node-baseline-dry-run-',
      async (tempDir) => {
        const sourceDir = path.join(tempDir, 'source');
        await writeTextFile(
          path.join(sourceDir, '00-sample.instructions.md'),
  ...
  it('uses check mode when both --check and --dry-run are provided', async () => {
    await withTempDir(
      'agent-toolkit-enforce-node-baseline-check-dry-run-',
      async (tempDir) => {
        const sourceDir = path.join(tempDir, 'source');
        await writeTextFile(
          path.join(sourceDir, '00-sample.instructions.md'),
